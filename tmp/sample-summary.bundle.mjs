...
function parseChasamJson(jsonText) {
  ...
  for (const item of parsed) {
    ...
    if (missing.length > 0) {
      console.warn(`\uB808\uCF54\uB4DC \uAC74\uB108\uB700 (\uB204\uB77D: ${missing.join(", ")})`);
      continue;
    }
    valid.push(item);
  }
...
function convertBatch(records2, createdBy2) {
  const results = records2.map((r) => convertRecord(r, createdBy2));
  return {
    results,
...
// scripts/sample-summary.ts
var filePath = "c:/_work_github/sajucube-chasam-importer/sample-data/Chasam_DB_20210113063107.txt";
var createdBy = "01012345678";
var text = readFileSync(filePath, "utf8");
var records = parseChasamJson(text);
var batch = convertBatch(records, createdBy);
var lunarCount = records.filter((record) => record.birthdayType === "L").length;
var unknownCount = records.filter((record) => record.birthdayType === "N").length;
var warningSamples = batch.results.filter((result) => result.warnings.length > 0).slice(0, 10).map((result) => ({
