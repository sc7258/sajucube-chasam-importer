...
- 병합 메모는 "병합 메모" 영역에서 줄바꿈과 출처 이름이 보이도록 표시한다.
- 같은 메모 내용은 중복 제거하고, 출처는 `[이름]` 형식으로 남긴다.
...
- 완료 화면에서 저장 결과 CSV 다운로드 가능
- 실패 건은 완료 화면에서 개별 재시도 가능
...
| `saving` | 저장 실행 중 (`N / M` 진행률 + 현재 이름 표시) |
| `done` | 결과 요약 표시 |
