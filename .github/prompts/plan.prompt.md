...
- [ ] **실제 차샘 DB 파일 end-to-end 테스트**
  ...
  - 실데이터 부재 시 로컬 스모크 테스트 스크립트로 핵심 변환 로직 점검
  - 현재 `npm run smoke:converter` 추가 및 통과

- [ ] **차샘 export BOM 처리**
  - 실제 sample 파일 앞의 UTF-8 BOM 제거 후 JSON 파싱 가능하도록 보정
  - sample-data/Chasam_DB_20210113063107.txt 기준으로 재검증

- [x] **저장 진행률 표시 개선**
  - "N / M 건 저장 중..." 실시간 카운터 표시
  - 현재 저장 중인 이름도 함께 표시
  - 위치: ReviewPage saving 단계
  - 빌드 성공 (`npm run build`)

- [x] **postPerson 실패 건 재시도**
  - 저장 실패한 개별 레코드에 "재시도" 버튼 추가
  - 재시도 후 완료 화면 요약과 CSV 대상 결과를 함께 갱신
...
- [x] **음력 윤달 처리 개선**
  - 음력 입력 시 평달/윤달 후보를 모두 계산
  - 원본 `ilju`와 더 잘 맞는 후보가 있으면 `isLeapMonth` 추론
  - 추론이 불가능하면 경고 문구와 수동 확인 안내 추가
  - 빌드 성공 (`npm run build`)
...
- [x] **문서 현행화 + 테스트 시나리오 정리**
  - `specs/`를 현재 구현 기준으로 업데이트
  - `.github/instructions` 설명과 현재 구현의 차이도 정리
  - 별도 테스트 시나리오 문서 추가로 검증 기준 명확화

- [x] **병합 그룹 설명/추천 개선**
  - ImportPage에 병합 검토 안내 배너 추가
  - ReviewPage 그룹 병합 탭에 병합 기준 안내 문구 추가
  - `detectVariants()` + `mergeVariants()` 흐름을 사용자 관점에서 설명
  - 빌드 성공 (`npm run build`)

- [x] **병합 시 메모/노트 merge 품질 개선**
  - 병합 시 동일 메모 중복 제거
  - `[이름] 메모` 형식 + 줄바꿈으로 출처 표시
  - 병합 그룹 UI에 "병합 메모" 영역 표시
  - 빌드 성공 (`npm run build`)

- [x] **저장 결과 CSV 내보내기**
  - ReviewPage 완료 화면에 CSV 다운로드 버튼 추가
  - 저장 결과, 검증 mismatch 여부, 비고를 CSV 행으로 저장
  - UTF-8 BOM 포함으로 한글 엑셀 호환 고려
  - 빌드 성공 (`npm run build`)
