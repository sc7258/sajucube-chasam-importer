...
**상황**: 차샘 DB에 윤달 정보가 없음  
**현재 처리**:
- 음력 입력이면 평달/윤달 후보를 모두 계산
- 원본 `ilju`와 더 잘 맞는 후보가 1개면 해당 후보로 `isLeapMonth` 추론
- 후보가 여러 개이거나 확정이 어려우면 경고를 남기고 기본 후보로 처리

**결과**: `isLeapMonth`를 일부 케이스에서 추론 가능  
**영향**: 음력 윤달 출생자의 양력 변환 날짜 오차 가능성 있음  
**주의**: 원본 데이터만으로 확정할 수 없는 케이스는 여전히 수동 확인 필요

---

### 병합 그룹 감지 (`detectVariants` / `mergeVariants`)

**상황**: 차샘 DB에서 동일 인물에 대해 이름 변형, 양력/음력 중복, 본원/허본 계열 데이터가 함께 존재하는 경우  
**감지**: `detectVariants()` — base name + additionalDates 교집합 + 윤달 인접 예외 기준으로 그룹화  
**처리**: `mergeVariants()`로 대표 레코드 1건만 유지하고, ReviewPage "그룹 병합" 탭에서 병합된 이름 목록을 보여줌  
**메모 처리**: `notes`는 중복 제거 후 `[이름] 메모` 형식으로 줄바꿈 병합
...
**처리**: `validateIlju` 실패 시 `warnings`에 추가되고 ReviewPage "경고" 탭에서 확인 가능.

---

### 차샘 JSON 파싱 특이사항

- 현재 구현은 JSON 배열 형식을 기준으로 처리
- 파싱 실패 시 ImportPage에서 오류 메시지 표시
- NDJSON 지원이 필요한 경우 실제 파일 샘플 기준으로 별도 검토 필요
