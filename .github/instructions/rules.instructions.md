...
코드 수정 작업을 시작하기 전에 반드시 다음 순서를 따른다:

1. **plan 업데이트 먼저** — `.github/prompts/plan.prompt.md`의 Todo 항목에 작업 내용을 추가하거나 상태를 갱신
2. **코드 수정 실행**
3. **빌드 확인** — `npm run build` 성공 후 plan의 해당 항목을 완료 처리 (`[ ]` → `[x]`)

> 이유: plan이 항상 실제 코드 상태와 동기화되어야 하므로, 작업 전 계획 반영 → 작업 후 완료 표시가 필수.
